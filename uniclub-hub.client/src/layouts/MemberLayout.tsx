...
function SectionLabel({ label }: { label: string }) {
  return (
    <div className="pt-3 pb-1">
      <p className="px-3 text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#9ca3af' }}>
        {label}
      </p>
    </div>
  )
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#f8fafc' }}>
      ...
      <aside className="w-60 flex-shrink-0 flex flex-col bg-white border-r border-gray-200">
        ...
        <div className="h-16 flex items-center gap-2.5 px-5 border-b border-gray-100 flex-shrink-0">
          ...
          <span className="font-semibold text-sm" style={{ color: '#111827' }}>UniClub Hub</span>
        </div>

        {/* Navigation */}
        <nav className="sidebar-nav flex-1 px-3 py-4 space-y-0.5 overflow-y-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: '#e5e7eb white' }}>
          {/* Chính */}
          ...
          {managedClubs.map(m => (
            <NavLink key={m.clubId} to={`/clubs/${m.clubId}/manage`} className={navCls}>
              <Users size={17} />
