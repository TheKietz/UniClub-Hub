...
import {
  getAdminNews, createAdminNews, updateAdminNews, deleteAdminNews,
  setAdminNewsPublish, uploadAdminNewsThumbnail,
} from '@/components/membership/services/adminNewsApi'
import type { PostResponse, CreatePostRequest, UpdatePostRequest } from '@/components/membership/services/postsApi'
...
const EMPTY_FORM: FormState = { title: '', content: '', category: 'News', publishDirectly: true }
...
export default function AdminNewsPage() {
  ...
  function openCreate() { setForm(EMPTY_FORM); setEditing(null); setCreating(true) }
  function openEdit(p: PostResponse) {
    setForm({ title: p.title, content: p.content, category: p.category as 'News' | 'Announcement', publishDirectly: false })
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        const dto: UpdatePostRequest = { title: form.title, content: form.content, category: form.category }
        await updateAdminNews(editing.id, dto)
        toast.success('Đã cập nhật tin')
      } else {
        const dto: CreatePostRequest = { title: form.title, content: form.content, category: form.category, publishDirectly: form.publishDirectly }
        await createAdminNews(dto)
        toast.success(form.publishDirectly ? 'Đã tạo và xuất bản tin' : 'Đã tạo bản nháp')
      }
      closeEditor()
      load(editing ? page : 1)
  ...
  async function handleTogglePublish(post: PostResponse) {
    setActionId(post.id)
    const publish = post.status !== 'Published'
    try {
      await setAdminNewsPublish(post.id, publish)
      toast.success(publish ? 'Đã xuất bản tin' : 'Đã chuyển về bản nháp')
      load(page)
    } catch {
  ...
  async function handleThumbnailChange(postId: number, file: File) {
    ...
    try {
      await uploadAdminNewsThumbnail(postId, file)
    ...
    <div style={{ minHeight: '100%', background: D.bg, padding: '28px 32px', fontFamily: "'Be Vietnam Pro', sans-serif" }}>
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: D.inkMuted, marginTop: 4, margin: 0 }}>
              Tin tức &amp; thông báo toàn trường, hiển thị ở trang Tin tức công khai
            </p>
          </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, color: D.inkMuted, marginLeft: 'auto' }}>{total} tin</span>
        </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <h2 style={{ fontSize: 16, fontWeight: 800, color: D.ink, margin: 0 }}>
                {editing ? 'Chỉnh sửa tin' : 'Tạo tin cấp trường'}
              </h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 180px', gap: 16, marginBottom: 14 }}>
              <div>
                ...
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                  placeholder="Nhập tiêu đề tin..." style={inputStyle} />
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: D.inkDim, display: 'block', marginBottom: 4 }}>Loại</label>
        ...
        <div style={{ background: D.card, borderRadius: D.radius, border: D.border, boxShadow: D.shadow(), overflow: 'hidden' }}>
          {loading ? (
            ...
            <div style={{ padding: 48, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, color: D.inkMuted, fontWeight: 600 }}>Chưa có tin cấp trường nào</div>
            </div>
          ) : posts.map((post, i) => {
              ...
              <div key={post.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4, background: st.bg, color: st.color }}>
                      {st.label}
                    </span>
                  </div>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  <button onClick={() => handleTogglePublish(post)} disabled={busy} style={{
                    height: 30, padding: '0 12px', borderRadius: D.pill,
                    border: `1.5px solid ${isPublished ? D.inkMuted : D.green}`,
                    background: isPublished ? D.bg : '#f0fdf4', color: isPublished ? D.inkDim : '#065f46',
        ...
        {deleteTarget && (
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 6px', fontSize: 16, fontWeight: 800, color: D.ink }}>Xóa tin?</h3>
