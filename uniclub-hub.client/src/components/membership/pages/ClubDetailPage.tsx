...
export default function ClubDetailPage() {
  ...
  const [resignPreference, setResignPreference] =
    useState<ResignationPreference>("LeaveClub");
  const [resignRequest, setResignRequest] =
  ...
  const isMember =
    ...
    (membership.status === MEMBERSHIP_STATUS.ACTIVE ||
      membership.status === MEMBERSHIP_STATUS.PROBATION);
  const isLeader =
    ...
    getUserResignations(user.id)
      .then((list) => {
        const pending = list.find(
          (r) => r.clubId === id && r.status === "Pending",
        );
  ...
  async function handleSubmitResignation() {
    setResigning(true);
    try {
      const result = await submitResignation(id, {
        preference: resignPreference,
      });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await submitApplication(id, {
        answers: schema ? answers : { note: answers["note"] ?? "" },
      });
      setSubmitted(true);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
              ...
              <div className="p-6">
                <div className="flex items-start gap-4">
                  {club.logoUrl ? (
                    ...
                    <div
                      className={`w-16 h-16 rounded-xl flex items-center justify-center text-white text-2xl font-bold flex-shrink-0 ${avatarColor}`}
                    >
                      {avatarLetter}
                    </div>
                  )}
                  <div className="min-w-0 flex-1 pt-0.5">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      <span className="text-xs font-mono px-2 py-0.5 rounded-md bg-gray-100 text-gray-500">
                        {club.code}
                      </span>
                      {club.categoryName && (
              ...
              <div className="p-5">
                {isMember ? (
                  ...
                  <div className="text-center py-2 space-y-2">
                    ...
                    <p className="text-xs text-gray-400">
                      CLB sẽ xem xét và thông báo sớm.
                    </p>
                  </div>
                ) : !isAuthenticated ? (
                  <div className="space-y-3">
                  ...
                  <p className="text-center text-sm text-gray-400 py-2">
                    CLB hiện không nhận thành viên mới.
                  </p>
                ) : (
                    ...
                    {schema && schema.fields.length > 0 ? (
                      ...
                      <div className="space-y-1.5">
                        ...
                        <textarea
                          ...
                          value={answers["note"] ?? ""}
                          onChange={(e) =>
                            setAnswers((p) => ({ ...p, note: e.target.value }))
                          }
      ...
      <Dialog open={resignOpen} onOpenChange={setResignOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {isLeader ? "Đệ đơn từ chức" : "Xác nhận rời CLB"}
            </DialogTitle>
          </DialogHeader>
          ...
          {isLeader ? (
            <div className="space-y-4 py-1">
              <p className="text-sm text-gray-500">
                ...
                , đơn từ chức của bạn cần được phê duyệt trước khi có hiệu lực.
              </p>
              <div className="space-y-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  Sau khi được duyệt, bạn muốn
                </p>
                {(
                  [
                    {
                      value: "LeaveClub",
                      label: "Rời CLB hoàn toàn",
                      desc: "Chấm dứt tư cách thành viên",
                    },
                    {
                      value: "BecomeMember",
                      label: "Trở thành thành viên thường",
                      desc: "Giữ lại tư cách thành viên, không còn vai trò quản lý",
                    },
                  ] as const
                ).map((opt) => (
                  <label
