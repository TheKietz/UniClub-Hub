...
import { ArrowLeft, Users, Building, Calendar, Phone, GraduationCap, CheckCircle2, Clock, XCircle, MessageCircle, LogOut, AlertCircle } from 'lucide-react'
                  ...
                  {departments.map(dept => (
                    <div key={dept.id} className="px-6 py-3.5 flex items-center justify-between">
                      <div>
                        ...
                        {dept.deptLeadName && (
                          <p className="text-xs text-gray-400 mt-0.5">Trưởng ban: {dept.deptLeadName}</p>
                        )}
                      </div>
                      <span className="text-xs text-gray-400 tabular-nums">{dept.memberCount} thành viên</span>
                    </div>
                  ))}
                </div>
              </div>
