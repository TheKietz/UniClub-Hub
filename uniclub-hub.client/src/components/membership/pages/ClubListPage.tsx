...
export default function ClubListPage() {
  ...
  const myClubIds = new Set(
    user?.memberships.filter(m => m.status === MEMBERSHIP_STATUS.ACTIVE).map(m => m.clubId)
  )
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1">
        <div className="max-w-5xl mx-auto px-6 pb-10 space-y-8">
          ...
          {/* Title */}
          <div>
            <h1 className="text-3xl font-bold pt-3" style={{ color: '#0f172a' }}>Khám phá CLB</h1>
            <p className="text-gray-500 mt-1">Tìm và đăng ký tham gia câu lạc bộ phù hợp với bạn</p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <form onSubmit={handleSearch} className="flex gap-2 flex-1 min-w-64">
              <div className="relative flex-1">
                ...
                <Input placeholder="Tìm tên CLB..." value={search}
                  ...
                  <div key={club.id}
                    ...
                    <div className="flex items-center gap-3">
                      {club.logoUrl ? (
                        ...
                        <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-white text-lg font-bold flex-shrink-0 ${avatarColor}`}>
                          {letter}
                        </div>
                      )}
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start gap-2">
                          <p className="font-semibold text-gray-900 leading-snug line-clamp-1 flex-1">{club.name}</p>
                          {isMember && (
                            <Badge variant="secondary" className="text-xs shrink-0 mt-0.5">Tham gia</Badge>
                          )}
                        </div>
                        <p className="text-xs text-gray-400 font-mono mt-0.5">{club.code}</p>
                      </div>
                    </div>
