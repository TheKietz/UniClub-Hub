  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 12, color: '#918c99', margin: 0 }}>
        Đang hiển thị <strong style={{ color: 'var(--c-ink)' }}>{shown}</strong> / <strong style={{ color: 'var(--c-ink)' }}>{total}</strong> {label}
      </p>
      {!done ? (
        <button
          onClick={onLoadMore}
          ...
          style={{
            ...
            background: loading ? 'var(--c-bg)' : 'var(--c-ink)',
            color: loading ? '#918c99' : '#facc15',
            border: '1.5px solid var(--c-ink)',
            boxShadow: loading ? 'none' : '2px 2px 0 var(--c-ink)',
        ...
        <span style={{
          ...
          background: 'var(--c-bg)', border: '1px solid #e8e3d6',
          letterSpacing: '.04em',
        }}>
          ✓ Đã hiển thị tất cả
        </span>
