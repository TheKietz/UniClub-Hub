...
import { getTasks, updateTaskStatus, getSprints } from '../services/operationsApi'
import type { TaskItem, TaskStatus, SprintItem, SprintStatus } from '../services/operations.types'
...
export default function KanbanPage() {
  ...
  useEffect(() => {
    ...
    conn.start()
      ...
      .catch(() => { /* hub optional — REST still works */ })
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{tasks.length} công việc</p>
        </div>
        ...
        {sprints.map(sprint => (
          ...
          >
            ...
            <span className={`ml-1.5 text-xs ${
              selectedSprintId === sprint.id ? 'text-indigo-200' : SPRINT_STATUS_COLOR[sprint.status]
            }`}>
      ...
      {selectedSprint && (
        <div className="mb-4 px-4 py-3 bg-white border rounded-xl flex items-center gap-4 text-sm text-gray-600 shadow-sm">
          <span className="font-semibold text-gray-800">{selectedSprint.name}</span>
          {selectedSprint.goal && (
            <span className="text-gray-500 truncate">{selectedSprint.goal}</span>
          )}
