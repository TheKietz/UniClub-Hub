    ...
    <div className="v3-page v3-enter" style={{
      ...
      <section style={{ padding: '52px 28px 44px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative' }}>
          ...
          {/* Graduation cap */}
          <div aria-hidden style={{
            position: 'absolute', top: 190, right: 90,
            fontSize: 38, animation: 'float 4.5s ease-in-out infinite 0.8s',
            userSelect: 'none',
          } as React.CSSProperties}>🎓</div>

          {/* Pencil */}
          <div aria-hidden style={{
            position: 'absolute', top: 100, right: 370,
            fontSize: 26, animation: 'float 3.8s ease-in-out infinite 1.4s',
            userSelect: 'none', transform: 'rotate(-20deg)',
          } as React.CSSProperties}>✏️</div>

          {/* Book */}
          <div aria-hidden style={{
            position: 'absolute', bottom: 10, right: 200,
            fontSize: 30, animation: 'float 5.2s ease-in-out infinite 2s',
            userSelect: 'none',
          } as React.CSSProperties}>📚</div>

          {/* Plus sign */}
          <div aria-hidden style={{
            position: 'absolute', top: 170, right: 440,
            color: C.ink, fontSize: 22, fontWeight: 900, opacity: 0.2,
            animation: 'float 3.2s ease-in-out infinite 0.3s',
          }}>+</div>

          {/* Small coral dot */}
          <div aria-hidden style={{
            position: 'absolute', bottom: 30, right: 360,
            width: 12, height: 12, borderRadius: 999,
            background: C.coral, opacity: 0.5,
            animation: 'float 4.2s ease-in-out infinite 2.3s',
          }} />

          {/* Rotated navy square */}
          <div aria-hidden style={{
            position: 'absolute', top: 45, right: 175,
            width: 16, height: 16, background: C.ink, opacity: 0.12,
          ...
          {/* Music note */}
          <div aria-hidden style={{
            position: 'absolute', bottom: 40, right: 460,
            fontSize: 22, color: C.indigo, opacity: 0.5,
          ...
          <Rv delay={60}>
            <h1 style={{
              ...
              Cổng thông tin<br />
              <span style={{ color: C.ink }}>câu lạc bộ</span>{' '}
          ...
          {[
            ...
            <Rv key={s.l} delay={i * 80}>
              <div style={{
                ...
                <div style={{ color: s.color, fontSize: 'clamp(40px, 5vw, 60px)', fontWeight: 900, letterSpacing: '-.04em', lineHeight: 1 }}>
                  {s.n}
                </div>
                <div style={{ color: C.bg, fontSize: 13, marginTop: 8, opacity: 0.65, fontWeight: 500, letterSpacing: '.02em' }}>{s.l}</div>
              </div>
            </Rv>
          ))}
        </div>
      ...
      {/* ─── Tất cả CLB ───────────────────────────────── */}
      {allPreview.length > 0 && (
        <section style={{ padding: '44px 28px 56px', borderTop: '2px dashed rgba(0,48,135,0.2)' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <Rv>
