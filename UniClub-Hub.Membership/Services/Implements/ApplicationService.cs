...
namespace UniClub_Hub.Membership.Services.Implements
{
    public class ApplicationService : IApplicationService
    {
        ...
        public async Task<IEnumerable<AdminApplicationDto>> GetAllByClubAsync(int clubId, string? status = null)
        {
        ...
        public async Task<ApplicationDto> SubmitAsync(int clubId, string userId, SubmitApplicationDto dto)
        {
            if (!await _db.Clubs.AnyAsync(c => c.Id == clubId))
                throw new KeyNotFoundException($"Không tìm thấy CLB với ID {clubId}.");
            ...
            foreach (var adminId in adminIds)
                await _notifications.SendAsync(adminId,
                    "Đơn đăng ký mới",
        ...
        public async Task<AdminApplicationDto> ReviewAsync(int clubId, int applicationId, ReviewApplicationDto dto, string reviewerId, bool isSuperAdmin)
        {
            var validStatuses = new[] { "Interview", "Accepted", "Rejected" };
            if (!validStatuses.Contains(dto.Status))
                throw new ArgumentException("Trạng thái không hợp lệ. Chỉ chấp nhận: Interview, Accepted, Rejected.");
            ...
            {
                "Interview" => ("Mời phỏng vấn", $"Đơn đăng ký vào CLB {clubName} của bạn được mời phỏng vấn."),
                "Accepted"  => ("Đơn được chấp nhận", $"Chúc mừng! Bạn đã được chấp nhận vào CLB {clubName}."),
                "Rejected"  => ("Đơn bị từ chối", $"Đơn đăng ký vào CLB {clubName} của bạn đã bị từ chối."),
        ...
        private static ApplicationDto ToDto(ClubApplication a) => new()
        {
            Id = a.Id,
            ClubId = a.ClubId,
        ...
        private static AdminApplicationDto ToAdminDto(ClubApplication a) => new()
        {
            Id = a.Id,
            ClubId = a.ClubId,
            ClubName = a.Club?.Name ?? "",
